...
correlation2 = cor(rank(traitmerge3[,"P.x"]),rank(tratmerge3[,"P.y"])) #calculate Spearman’s rank correlation coefficient
...
mergefinal = traitmerge[,c("SNP.x", "freq.a.x","freq.b.x", "CHR.x", "BP.x", "effect_allele.x", "other_allele.x","sexdifft","pvalt3","effect.y", "stderr.y","effect.x", "stderr.x")] #prune the data table. 
traitmergefinal = traitmergefinal[order(traitmergefinal$pvalt),] ) #order according to p-values in ascending order.
write.table(EQmergefinal2, file = "traitsexdifference.txt", row.names = F, col.names = T, quote = F) #write the file as a text file. 
